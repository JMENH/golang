...
var uintsize = 32 << (^uint(0) >> 63) // the effective size of `uint` in bits, 32 or 64.
...
func (s *IntSet) Clear() {
	s.words = make([]uint, 0, len(s.words))
}
...
func (s *IntSet) IntersectWith(t *IntSet) {
	var temp = make([]uint, len(s.words))
	for i, tword := range t.words {
		if i < len(temp) {
